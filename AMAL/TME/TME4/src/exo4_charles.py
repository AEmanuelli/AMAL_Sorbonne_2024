...
from utils_charles import RNN, device
...
# Training function
def train(model_name, total_epoch=33, save_every_n_epochs=5):
    ...
    for epoch in tqdm(range(start_epoch, total_epoch)):
        epoch_loss = 0
        # epoch_loss_test = 0
        with torch.profiler.profile(schedule=torch.profiler.schedule(wait=2, warmup=2, active=6, repeat=2),
    ...
    torch.save(model.state_dict(), PATH + f"{model_name}_final.pt")
...
def generate(model_name, lenght=45, prompt='Trump:'):
    ...
    embeding.load_state_dict(torch.load(PATH+f"{model_name}_embeding.pt"))
    for _ in range(50):
        h = torch.zeros(
